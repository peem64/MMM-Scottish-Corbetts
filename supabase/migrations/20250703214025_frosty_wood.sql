/*
  # Safely insert corbetts data with duplicate handling

  1. Data Population
    - Insert sample corbetts data with comprehensive information
    - Uses ON CONFLICT to handle any existing data safely
...
-- Add some additional corbetts that might not exist yet
INSERT INTO corbetts (
  name, location, region, height, prominence, classification, estimated_time, 
  description, difficulty_rating, popular_routes, best_seasons, grid_reference, 
  nearest_town, parking_info, image_filename
...
(
  ...
  'Corbett', 
  '2-3 hours', 
  'A distinctive and dramatic peak with spectacular sandstone pinnacles. Despite its modest height, Stac Pollaidh offers one of the most rewarding short walks in Scotland with incredible views over the Assynt landscape.',
  3,
  ARRAY['Path from car park', 'Pinnacle traverse (scrambling)', 'Eastern approach'],
  ARRAY['Spring', 'Summer', 'Autumn'],
  'NC107107',
  'Achiltibuie',
  'Dedicated car park on minor road from A835. Well signposted.',
  'stac-pollaidh.jpg'
),
(
  'Cul Mor', 
  ...
  'Corbett', 
  '5-6 hours', 
  'A magnificent isolated peak rising dramatically from the wild landscape of Assynt. The mountain offers spectacular views and a true sense of wilderness, with excellent walking on good paths through stunning Highland scenery.',
  3,
  ARRAY['Knockan Crag approach', 'Elphin path', 'Circuit via Cul Beag'],
  ARRAY['Late Spring', 'Summer', 'Early Autumn'],
  'NC161120',
  'Elphin',
  'Parking at Knockan Crag visitor centre or roadside near Elphin.',
  'cul-mor.jpg'
)
ON CONFLICT (name) DO NOTHING;